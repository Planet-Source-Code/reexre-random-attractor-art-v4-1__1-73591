...
Private Sub picDraw_MouseDown(Button As Integer, Shift As Integer, X As Single, y As Single)
    ...
    FN = mItem(mySel).strFile

    ii = InStrRev(FN, "\")
    FN = Right$(FN, Len(FN) - ii)
    FN = Left$(FN, Len(FN) - 4) & ".txt"
    sPic = App.Path & "\Thumbnails\" & Left$(FN, Len(FN) - 4) & ".jpg"


    LoadParam FN

    If Mode3D Then
        If GenerateCURVE3D(frmMAIN.cmbATTmode.ListIndex) Then
            ScreenPOINTS
            MsgBox FN & "  Loaded." & vbCrLf & vbCrLf & "3D Ready to render...", vbInformation
            Success = True
        Else
            MsgBox FN & "3D Curve for some reason cannot be generated...", vbCritical
        End If
    Else
        If GenerateCURVE(frmMAIN.cmbATTmode.ListIndex) Then
            ScreenPOINTS
            MsgBox FN & "  Loaded." & vbCrLf & vbCrLf & "Ready to render...", vbInformation
            Success = True
        Else
            MsgBox FN & " Curve for some reason cannot be generated...", vbCritical
        End If
    End If

    If Success Then
        frmMAIN.PicSave = LoadPicture(sPic)
        frmMAIN.PicSave.Refresh
        SetStretchBltMode frmMAIN.PIC.hdc, 4
        StretchBlt frmMAIN.PIC.hdc, 0, 0, frmMAIN.PIC.Width, frmMAIN.PIC.Height, _
                   frmMAIN.PicSave.hdc, 0, 0, frmMAIN.PicSave.Width, frmMAIN.PicSave.Height, vbSrcCopy
    End If



    If Success Then Unload Me Else: Draw
